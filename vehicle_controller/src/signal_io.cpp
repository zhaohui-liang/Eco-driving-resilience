  ...
  controller_(controller)
{
    ...
    node_->declare_parameter("disturbance", 0.0);
    disturbance_ = node->get_parameter("disturbance").as_double();
...
if (gps_distance_ < 140)
    ...
    else
    {
        ...
        {
        signal_phase_ = 3;  // Red
        signal_time_left_ = static_cast<int>((red_duration_ - cycle_time) * 10) + disturbance_*10;
        } else if (cycle_time < red_duration_ + green_duration_) {
            signal_phase_ = 6;  // Green
            signal_time_left_ = static_cast<int>((red_duration_ + green_duration_ - cycle_time) * 10) + disturbance_*10;
        } else {
            signal_phase_ = 8;  // Yellow
            signal_time_left_= static_cast<int>((cycle - cycle_time) * 10) + disturbance_*10;
        }
    }
